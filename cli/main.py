    ...
    try:
        ...
        for field in required_fields:
            if field not in memory_data:
                console.print(
                    f"[red]Error: Missing required field '{field}' in memory file[/red]"
                )
                sys.exit(1)
        ...
        with httpx.Client() as client:
            response = client.post(
                f"{server_url}/memory/", json=memory_data, timeout=30.0
            )
        ...
        console.print(
            f"[red]Error: Server returned {e.response.status_code}: {e.response.text}[/red]"
        )
        sys.exit(1)
    except httpx.RequestError as e:
        console.print(
            f"[red]Error: Failed to connect to server at {server_url}: {e}[/red]"
        )
        sys.exit(1)
...
def recall(
    ...
    try:
        ...
        with httpx.Client() as client:
            response = client.get(
                f"{server_url}/memory/search", params=params, timeout=30.0
            )
            response.raise_for_status()
            ...
            if output_format == "rich":
                ...
                for i, memory in enumerate(result["results"], 1):
                    score_color = (
                        "green"
                        if memory["score"] > 0.8
                        else "yellow" if memory["score"] > 0.6 else "red"
                    )

                    console.print(f"\n[bold]{i}. Memory ID: {memory['id']}[/bold]")
                    console.print(
                        f"[{score_color}]Relevance: {memory['score']:.3f}[/{score_color}]"
                    )
                ...
                for i, memory in enumerate(result["results"], 1):
                    console.print(
                        f"{i}. ID: {memory['id']} (Score: {memory['score']:.3f})"
                    )
                    console.print(f"   Project: {memory['entry']['project']}")
                    console.print(f"   Category: {memory['entry']['category']}")
...
def list_memories(project: Optional[str], limit: int, server: Optional[str]):
    ...
    try:
        ...
        with httpx.Client() as client:
            response = client.get(f"{server_url}/memory/", params=params, timeout=30.0)
            response.raise_for_status()
    ...
    try:
        with httpx.Client() as client:
            ...
            stats_response = client.get(
                f"{server_url}/memory/stats/collection", timeout=10.0
            )
