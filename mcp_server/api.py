...
from .models import MemoryEntry, MemorySearchParams, MemorySearchResult, MemoryUpdateRequest
...
async def create_memory(entry: MemoryEntry):
    ...
    try:
        ...
        if not success:
            logger.warning(f"Memory {memory_id} saved to file but failed to index in vector DB")
