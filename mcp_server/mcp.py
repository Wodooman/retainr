...
async def _tool_search_memories(args: Dict[str, Any]) -> MCPToolCallResponse:
    ...
    try:
        ...
        return MCPToolCallResponse(content=[{"type": "text", "text": response_text}])
...
async def _tool_list_memories(args: Dict[str, Any]) -> MCPToolCallResponse:
    ...
    try:
        ...
        if not files:
            ...
            return MCPToolCallResponse(
                content=[{"type": "text", "text": f"No memories found{filter_text}"}]
            )
...
async def _tool_update_memory(args: Dict[str, Any]) -> MCPToolCallResponse:
    ...
    try:
        ...
        if not file_path:
            return MCPToolCallResponse(
                content=[
                    {"type": "text", "text": f"Memory with ID {memory_id} not found"}
                ],
                isError=True,
            )
        ...
        if not success:
            return MCPToolCallResponse(
                content=[
                    {"type": "text", "text": f"Failed to update memory {memory_id}"}
                ],
                isError=True,
            )
        ...
        return MCPToolCallResponse(
            content=[{"type": "text", "text": f"Memory {memory_id} marked as {status}"}]
        )
