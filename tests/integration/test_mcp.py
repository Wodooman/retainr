...
class TestMCPIntegration:
    ...
    async def test_mcp_initialize(self, base_url):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{base_url}/initialize", json=init_data, timeout=30.0
            )
    ...
    async def test_mcp_tools_list(self, base_url):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{base_url}/tools/list", json={}, timeout=30.0
            )
    ...
    async def test_mcp_tool_call_save_memory(self, base_url):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{base_url}/tools/call", json=tool_call, timeout=30.0
            )
    ...
    async def test_mcp_tool_call_search_memories(self, base_url):
        ...
        async with httpx.AsyncClient() as client:
            ...
            save_response = await client.post(
                f"{base_url}/tools/call", json=save_call, timeout=30.0
            )
            ...
            search_call = {
                ...
                "arguments": {"query": "searchable memory", "top": 3},
            }

            search_response = await client.post(
                f"{base_url}/tools/call", json=search_call, timeout=30.0
            )
    ...
    async def test_mcp_tool_call_list_memories(self, base_url):
        ...
        tool_call = {"name": "list_memories", "arguments": {"limit": 5}}
    ...
    async def test_mcp_resources_list(self, base_url):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{base_url}/resources/list", json={}, timeout=30.0
            )
    ...
    async def test_mcp_full_workflow(self, base_url):
        ...
        async with httpx.AsyncClient() as client:
            ...
            search_response = await client.post(
                ...
                json={
                    "name": "search_memories",
                    "arguments": {"query": "workflow test", "project": "workflow-test"},
                },
                timeout=30.0,
            )
            assert search_response.status_code == 200
